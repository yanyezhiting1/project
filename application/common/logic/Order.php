...
//       \mall\queue\QueueClient::push('sendMemberMsg', $param);
        $list = db('ordergoods')->where('order_id',$order_info['order_id'])->select();
        foreach($list as $v){

            $message['to_member_id'] =  $order_info['buyer_id'];
            $message['goods_id'] = $v['goods_id'];
            $message['message_type']=2;
            $message['message_title']='订单派送中';
            $message['message_body']='订单已发货，请留意物流信息，注意查收';
            $message['message_time']=$time;
            $message['order_sn']=$order_info['order_sn'];
