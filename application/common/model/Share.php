...
class  Share extends Model {
     ...
     * @return array 二维数组
     */
    public function getShareList($condition = array(), $page = '', $limit = '', $orderby = 'id desc') {
        if ($page) {
            $result = db('share')->where($condition)->order($orderby)->paginate($page, false, ['query' => request()->param()]);
            $this->page_info = $result;
            return $result->items();
        } else {
            return db('share')->where($condition)->order($orderby)->select();
        }
    }
