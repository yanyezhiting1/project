...
class  Membervip extends Model {
    ...
    public function addAdv($data) {
        $result = db('membervip')->insertGetId($data);
    ...
    public function delAdv($id) {
        $condition['id'] = $id;
//        $adv = db('membervip')->where($condition)->find();
//        if ($adv) {
//            // drop cache
//            $apId = (int) $adv['ap_id'];
//            dkcache("adv/{$apId}");
...
//        @unlink(BASE_UPLOAD_PATH . DS . ATTACH_ADV. DS .$adv['adv_code']);
        return db('membervip')->where($condition)->delete();
    }
    ...
    public function getOneVip($condition = array()) {
        return db('membervip')->where($condition)->find();
    }
     ...
     * @return array 二维数组
     */
    public function getVipList($condition = array(), $page = '', $limit = '', $orderby = 'id desc') {
        if ($page) {
            $result = db('membervip')->where($condition)->order($orderby)->paginate($page, false, ['query' => request()->param()]);
            $this->page_info = $result;
            return $result->items();
        } else {
            return db('membervip')->where($condition)->order($orderby)->select();
        }
    }
     ...
     * @return bool
     */
    public function editVip($data) {
        $adv_array = db('membervip')->where('id', $data['id'])->find();
//        if ($adv_array) {
//            // drop cache
//            $apId = (int) $adv_array['id'];
//            dkcache("adv/{$apId}");
//        }
        return db('membervip')->where('id', $data['id'])->update($data);
    }
