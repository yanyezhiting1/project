...
class  Supply extends Model {
     ...
     * @return array 二维数组
     */
    public function getSupplyList($condition = array(), $page = '', $limit = '', $orderby = 'id desc') {
        if ($page) {
            $result = db('supply')->where($condition)->order('id DESC')->paginate($page, false, ['query' => request()->param()]);
            $this->page_info = $result;
            return $result->items();
        } else {
            return db('supply')->where($condition)->order($orderby)->select();
        }
    }
