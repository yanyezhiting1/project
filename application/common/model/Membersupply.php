...
class  Membersupply extends Model {
    ...
    public function addAdv($data) {
        $result = db('membersupply')->insertGetId($data);
        $apId = (int) $data['ap_id'];
        dkcache("adv/{$apId}");
        return $result;
    }
    ...
    public function delAdv($id) {
        $condition['id'] = $id;
        $adv = db('membersupply')->where($condition)->find();
        if ($adv) {
            // drop cache
            $apId = (int) $adv['ap_id'];
            dkcache("adv/{$apId}");
        ...
        return db('membersupply')->where($condition)->delete();
    }
    ...
    public function getOneAdv($condition = array()) {
        return db('membersupply')->where($condition)->find();
    }
     ...
     * @return array 二维数组
     */
    public function getAuthList($condition = array(), $page = '', $limit = '', $orderby = 'id desc') {
        if ($page) {
            $result = db('membersupply')->where($condition)->order($orderby)->paginate($page, false, ['query' => request()->param()]);
            $this->page_info = $result;
            return $result->items();
        } else {
            return db('membersupply')->where($condition)->order($orderby)->select();
        }
    }
    ...
    public function mbadvlist($condition,$orderby='adv_sort desc'){
         return db('membersupply')->alias('a')->join('__ADVPOSITION__ n','a.ap_id=n.ap_id')->where($condition)->order($orderby)->select();
    }
    ...
    public function editAdv($data) {
        $adv_array = db('membersupply')->where('id', $data['id'])->find();
        if ($adv_array) {
            // drop cache
            $apId = (int) $adv_array['ap_id'];
            dkcache("adv/{$apId}");
        }
        return db('membersupply')->where('id', $data['id'])->update($data);
    }
