...
class  Offer extends Model {
    ...
    public function addOffer($data) {
        $result = db('offer')->insertGetId($data);
        return $result;
    }
     ...
     * @return array 二维数组
     */
    public function getOfferList($condition = array(), $page = '', $limit = '', $orderby = 'id desc') {
        if ($page) {
            $result = db('offer')->where($condition)->order($orderby)->paginate($page, false, ['query' => request()->param()]);
            $this->page_info = $result;
            return $result->items();
        } else {
            return db('offer')->where($condition)->order($orderby)->select();
        }
    }
