...
class  Goodsclass extends AdminControl {
    ...
    public function goods_class_add() {
        ...
        if (!request()->isPost()) {
            ...
            $temp = db('goodsclass')->where('gc_parent_id',$insert_array['gc_parent_id'])->where('gc_name',$insert_array['gc_name'])->find();
            if($temp !=''){
                $this->error('分类名称不能重复');
            }
            //验证数据  END
            ...
            if ($result) {
                if ($insert_array['gc_parent_id'] == 0) {
                    ...
                    if (!empty($_FILES['pic']['name'])) {//上传图片
                        ...
                        $pic_name = BASE_UPLOAD_PATH . '/' . ATTACH_COMMON . '/category-pic-' . $result.$time . '.jpg';
                        $where['gc_id'] = $result;
                        $update_array['pic'] = $pic_name;
    ...
    public function goods_class_edit() {
        ...
        if (!request()->isPost()) {
            ...
            $time = time();
            $pic_name = BASE_UPLOAD_PATH . '/' . ATTACH_COMMON . '/category-pic-' . $class_array['gc_id'] .$time. '.jpg';

            	
                $class_array['pic'] = UPLOAD_SITE_URL . '/' . ATTACH_COMMON . '/category-pic-' . $class_array['gc_id'] .$time. '.jpg';
            ...
            if (!empty($_FILES['pic']['name'])) {//上传图片
                ...
                if (!empty($_FILES['pic']['name'])) {//上传图片
                    $file = request()->file('pic');
                    $file->validate(['ext' => ALLOW_IMG_EXT])->move($upload_file, 'category-pic-' . $gc_id .$time. '.jpg');
                    $pic_name = BASE_UPLOAD_PATH . '/' . ATTACH_COMMON . '/category-pic-' . $gc_id .$time. '.jpg';
                    
                    $update_array2['pic'] = $pic_name;
                    $goodsclass_model->editGoodsclass($update_array2, $where);
                }
            }
    ...
    public function nav_edit() {
        ...
        if (request()->isPost()) {
            ...
            if (isset($class_id_array)&&is_array($class_id_array)) {
                $update['goodscn_classids'] = implode(',', $class_id_array);
            }
            $brand_id_array = input('post.brand_id/a');
            if (isset($brand_id_array)&&is_array($brand_id_array)) {
