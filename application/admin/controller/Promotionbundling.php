...
class  Promotionbundling extends AdminControl
{
    ...
    public function bundling_add() {
        ...
        if (request()->isPost()) {
            ...
            $off =  intval(input('post.off'));
            if($off>=10){
                ds_json_encode(10001,'折扣为不能大于10的数字');
            }
            if($off==0){
                ds_json_encode(10001,'折扣不能为0');
            }

            if ($bl_id <= 0) {
                $data['image'] = input('post.image_path');
                $data['image2'] = input('post.image_path2');
                $data['image3'] = input('post.image_path3');
                $data['image4'] = input('post.image_path4');
                $data['image5'] = input('post.image_path5');
                $data['bl_name'] = input('post.bundling_name');
                $data['off'] = input('post.off');
                ...
                $data['bl_content'] = input('post.bl_content');
            ...
            if (!empty($goods_array) && is_array($goods_array)) {
                foreach ($goods_array as $key => $val) {
                    ...
                    if($key==0){
                        db('pbundling')->where('bl_id',$bl_id)->update(['goods_id'=>$goods_info['goods_id']]);
                    }
                    $data = array();
                    $data['bl_id'] = $bl_id;
            ...
            $class_list = db('goodsclass')->where('gc_parent_id',133)->select();
