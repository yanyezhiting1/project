 ...
 * 控制器
 */
class  Circle extends AdminControl {
    ...
    public function index() {
        ...
        if (!request()->isPost()) {
            ...
            $circle_info = $circle_model->getCircleList($condition, 20, '', 'id DESC');


            foreach($circle_info as $k=>$v){
                $image = $circle_info[$k]['image'];
                if($image !=''){
                    $circle_image =explode(',',$image);
                }else{
                    $circle_image='';
                }
                ...
                $circle_info[$k]['area_name'] = $area.$city.$district;
                if($circle_image!=''){
                    foreach($circle_image as $kk=>$vv){

                        $circle_info[$k]['circle_image'][] = $vv;

                    }
                }else{
                    $supply_info[$k]['circle_image']='';
                }


            }

            $this->assign('supply_info', $circle_info);

            $this->assign('show_page', $circle_model->page_info->render());

            $this->assign('filtered', $condition ? 1 : 0); //是否有查询条件
            $this->setAdminCurItem('adv');
            return $this->fetch('circle_index');
        }
    }
...
//               $this->success(lang('adv_change_succ'), input('post.ref_url'));
    ...
    public function supply_check() {
        ...
        $result = db('circle')->where('id',$id)->update(['state'=>$state]);
        if (!$result) {
