 ...
 * 控制器
 */
class  Share extends AdminControl {
    ...
    public function index() {
        ...
        if (!request()->isPost()) {
            ...
            $share_info = $share_model->getshareList($condition, 20, '', 'id DESC');

            $this->assign('share_info', $share_info);

            $this->assign('show_page', $share_model->page_info->render());

            $this->assign('filtered', $condition ? 1 : 0); //是否有查询条件
            $this->setAdminCurItem('adv');
            return $this->fetch('share_index');
        }
    }
...
//               $this->success(lang('adv_change_succ'), input('post.ref_url'));
    ...
    public function share_check() {
        ...
        $result = db('share')->where('id',$id)->update(['state'=>$state]);
        if (!$result) {
