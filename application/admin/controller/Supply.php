 ...
 * 控制器
 */
class  Supply extends AdminControl {
...
//    }
    ...
    public function index() {
        ...
        if (!request()->isPost()) {
            ...
            $supply_info = $supply_model->getSupplyList($condition, 20, '', 'id DESC');
            foreach($supply_info as $k=>$v){
                $gc_id_1 = $v['gc_id_1'];
                $gc_id_2 = $v['gc_id_2'];
                $gc_id_3 = $v['gc_id_3'];
                $gc_name =  db('goodsclass')->where('gc_id',$gc_id_1)->value('gc_name');
                if($gc_id_2 !=''){
                    $gc_name .=  db('goodsclass')->where('gc_id',$gc_id_2)->value('gc_name');
                }
                if($gc_id_3 !=''){
                    $gc3_name =  db('goodsclass')->where('gc_id',$gc_id_3)->value('gc_name');
                }else{
                    $gc3_name='';
                }


                $supply_info[$k]['gc_name'] = $gc_name;

                $supply_info[$k]['title'] = $v['name'];
                $supply_info[$k]['material2_name'] = $gc3_name;
            }

            $this->assign('supply_info', $supply_info);

            $this->assign('show_page', $supply_model->page_info->render());
            
            $this->assign('filtered', $condition ? 1 : 0); //是否有查询条件
            $this->setAdminCurItem('adv');
            return $this->fetch('supply_index');
        }
    }
...
//               $this->success(lang('adv_change_succ'), input('post.ref_url'));
    ...
    public function supply_check() {
        ...
        $result = db('supply')->where('id',$id)->update(['state'=>$state]);
        if (!$result) {
