$(function(){
    ...
    $('#goods_image2').fileupload({
        dataType: 'json',
        url: ADMINSITEURL + '/Goodsadd/image_upload.html?upload_type=uploadedfile',
        formData: {name:'goods_image2'},
        add: function (e,data) {
            $('img[dstype="goods_image2"]').attr('src', ADMINSITEROOT + '/images/loading.gif');
            data.submit();
        },
        done: function (e,data) {
            var param = data.result;
            if (typeof(param.error) != 'undefined') {
                alert(param.error);
                $('img[dstype="goods_image2"]').attr('src',DEFAULT_GOODS_IMAGE);
            } else {
                $('input[dstype="goods_image2"]').val(param.name);
                $('img[dstype="goods_image2"]').attr('src',param.thumb_name);
            }
        }
    });
    $('#goods_image3').fileupload({
        dataType: 'json',
        url: ADMINSITEURL + '/Goodsadd/image_upload.html?upload_type=uploadedfile',
        formData: {name:'goods_image3'},
        add: function (e,data) {
            $('img[dstype="goods_image3"]').attr('src', ADMINSITEROOT + '/images/loading.gif');
            data.submit();
        },
        done: function (e,data) {
            var param = data.result;
            if (typeof(param.error) != 'undefined') {
                alert(param.error);
                $('img[dstype="goods_image3"]').attr('src',DEFAULT_GOODS_IMAGE);
            } else {
                $('input[dstype="goods_image3"]').val(param.name);
                $('img[dstype="goods_image3"]').attr('src',param.thumb_name);
            }
        }
    });
    $('#goods_image4').fileupload({
        dataType: 'json',
        url: ADMINSITEURL + '/Goodsadd/image_upload.html?upload_type=uploadedfile',
        formData: {name:'goods_image4'},
        add: function (e,data) {
            $('img[dstype="goods_image4"]').attr('src', ADMINSITEROOT + '/images/loading.gif');
            data.submit();
        },
        done: function (e,data) {
            var param = data.result;
            if (typeof(param.error) != 'undefined') {
                alert(param.error);
                $('img[dstype="goods_image4"]').attr('src',DEFAULT_GOODS_IMAGE);
            } else {
                $('input[dstype="goods_image4"]').val(param.name);
                $('img[dstype="goods_image4"]').attr('src',param.thumb_name);
            }
        }
    });
    $('#goods_image5').fileupload({
        dataType: 'json',
        url: ADMINSITEURL + '/Goodsadd/image_upload.html?upload_type=uploadedfile',
        formData: {name:'goods_image5'},
        add: function (e,data) {
            $('img[dstype="goods_image5"]').attr('src', ADMINSITEROOT + '/images/loading.gif');
            data.submit();
        },
        done: function (e,data) {
            var param = data.result;
            if (typeof(param.error) != 'undefined') {
                alert(param.error);
                $('img[dstype="goods_image5"]').attr('src',DEFAULT_GOODS_IMAGE);
            } else {
                $('input[dstype="goods_image5"]').val(param.name);
                $('img[dstype="goods_image5"]').attr('src',param.thumb_name);
            }
        }
    });
